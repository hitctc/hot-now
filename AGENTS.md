...
- AI 时间线第一阶段已落地：官方源白名单先收 `OpenAI News` 和 `Google AI Blog`，手动采集后写入 `ai_timeline_events`，页面 `/ai-timeline` 支持事件类型、公司和标题搜索筛选；当前不收媒体报道、非官方传闻，也不参与 `AI 新讯 / AI 热点` 的普通内容流
