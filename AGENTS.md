...
- 创作列表性能治理已接入：数据库迁移 `46` 增加素材方向/创建时间、成品方向/置顶/创建时间和推送日志文章/状态复合索引；Vue 列表使用 `view=summary` 只读取表格字段，每页默认 `30` 条，素材展开与文章弹窗再按 ID 读取完整详情。未传 `view=summary` 的外部 Agent 仍获得完整响应，不得破坏这条兼容边界。
