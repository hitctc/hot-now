...
- 自动化使用一个全局模式：`running`、`paused`、`emergency_stopped`；阶段开关仍独立存在。短内容的素材采集入库与自动写作是两个独立阶段，采集和写作各自有调度间隔，写作只消费当前采集批次，不自动补写历史 `ready` 素材。暂停只阻止自动任务，紧急停止还会取消未开始的自动队列项；两者都不影响人工写作。短内容素材表格的人工写作必须进入 Hermes 全局文章队列，并携带 HotNow 平台素材 ID，供右侧队列浮层展示素材、形态、阶段和最终成品；HotNow 不为此创建平行队列。
