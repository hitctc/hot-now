...
- `/creative/source-items`：公众号素材工作台；质量闸门主动中止后保持 `skipped`，并在素材记录中持久化停止阶段、阶段名称、具体原因和时间。鼠标移入带 `ⓘ` 的停止状态标签可查看说明，展开区显示完整信息；素材重新开始写作时清除旧说明
...
- 公众号写作管线 v2 已接入：`creative_finished_articles` 保存 `pipeline_version`、读者任务、普通人相关性、证据包、读者价值方案、事实骨架、口述底稿、分组标题候选、事实来源清单和标题确认状态；旧文章的确认状态默认兼容为已确认。公众号写作入口不再展示或传递 A/B/C，短内容入口不受影响。效果反馈指标继续保存于 `performance_*`，前端由 `ArticlePerformanceFeedbackModal.vue` 快速录入；当前不做自动抓取、提醒和看板。权威需求与实施状态见 `/Users/tc-nihao/.hermes/scripts/prd/公众号写作管线-v2.md`
