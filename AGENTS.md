...
- AI 时间线第一阶段已落地并扩展为多官方源采集：官方源白名单覆盖 `OpenAI News`、`Google AI Blog`、`Gemini API Release Notes`、`Claude Code Changelog`、`Qwen Blog`，以及 `Qwen / DeepSeek / Meta Llama / Mistral / ByteDance Seed` 的 Hugging Face 官方组织模型列表；手动采集后写入 `ai_timeline_events`，页面 `/ai-timeline` 支持事件类型、公司和标题搜索筛选；当前不收媒体报道、非官方传闻，也不参与 `AI 新讯 / AI 热点` 的普通内容流。OpenAI API changelog 暂不接入，因为静态请求会被 Cloudflare challenge 阻断；GitHub releases 暂不接入，因为未认证 API 容易触发 rate limit。
