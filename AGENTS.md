...
- 当前数据源：内置 RSS 库已扩展到 `21` 个，覆盖聚合日报、国际官方 AI 博客、国内科技媒体、创投资讯、开发者社区与综合新闻；Twitter 已拆成两类独立来源类型：账号采集配置保存在 `twitter_accounts`，关键词搜索配置保存在 `twitter_search_keywords`，都只支持后台手动执行，完整清单和边界见本文第 `9` 节阶段快照和 `README.md`
...
- `/settings/sources`：统一站点数据迭代收集页（登录后，由 `Vue 3 + Ant Design Vue` 驱动，可启用/停用 source、切换“选中该来源时全量展示”，并支持可视化新增 / 编辑 / 删除自定义 RSS 来源与微信公众号桥接来源；新增 RSS 时只需要填写 `RSS URL`，新增公众号时只需要填写 `公众号名称`，文章链接可选但建议一起填，其余 `kind / 来源名称 / 来源主页 / bridge 细节` 都由系统内部自动生成；来源保存成功后会立即自动补拉这条来源的首批内容；页面同时提供独立 Twitter 账号分区和独立 Twitter 关键词搜索分区，可分别新增 / 编辑 / 删除 / 启停 / 手动采集，并查看最近成功和最近结果状态；Twitter 关键词分区额外支持 `采集启用`、`展示启用` 双开关，分类统一映射为中文文案；页面会展示总条数、今天发布、今天抓取，以及 `AI 新讯 / AI 热点` 入池与展示统计，并按真实调度回显 `下一次采集：18:40（还有 6 分钟）` 这类分钟级文案）
...
5. 进入 `/settings/sources` 或 legacy `/control`，先手动执行一次 RSS / 微信公众号采集；如果已配置 `TWITTER_API_KEY`，再到 `/settings/sources` 的 Twitter 分区单独执行一次 Twitter 账号采集，并确认账号“最近成功 / 最近结果”回写；如需验证关键词搜索，再新增一个已启用且可展示的 Twitter 关键词，执行一次“手动采集 Twitter 关键词”，确认关键词“最近成功 / 最近结果”回写，并到 `/ai-new`、`/ai-hot` 检查结果可见性；需要验证发信时，再单独触发一次“发送最新报告”
...
- Twitter 推文统一写入现有 `content_items`，并在 `content_items.metadata_json` 保存 metrics、author 和 collector 信息；为满足 `content_items.source_id` 外键，会维护两个隐藏的内部聚合 source：`twitter_accounts` 和 `twitter_keyword_search`，它们都不承载实际配置，也不会出现在 `/settings/sources` 的普通 source 库存表中；账号聚合 source 在内容页默认可见，关键词聚合 source 的内容是否展示则额外受关键词 `is_visible` 命中关系控制
- 采集和发信已拆成两个独立功能：默认配置下采集每 `10` 分钟执行一次，发信每天 `10:00` 执行一次；两者都支持手动触发，并共用同一把运行锁
- 系统菜单已收口到多源语义：`/settings/sources` 支持 source 启用/停用、source 级“选中时全量展示”策略、逐 source 最近抓取状态展示，以及统一站点内手动执行 RSS / 微信公众号采集、手动发送最新报告；同时支持可视化新增 / 编辑 / 删除自定义 RSS 来源与公众号来源，其中 RSS 只要求录入 `RSS URL`，公众号只要求录入 `公众号名称` 和可选的 `公众号文章链接`，其余内部字段由系统自动生成与解析；页面还支持独立维护 Twitter 账号列表并单独执行 Twitter 手动采集，也支持独立维护 Twitter 关键词列表并单独执行 Twitter 关键词搜索，两者都不会混入普通 source 库存表；来源保存成功后会立即自动补拉这条来源的首批内容；本地开发默认由仓库内置公众号解析 sidecar 处理公众号输入，后续如需 IP 隔离再切远端 relay；`/settings/view-rules` 现在会解释并控制 `AI 新讯 / AI 热点` 的真实筛选开关，同时保留 `反馈池` 与 `LLM 设置（暂未使用）`；当前登录用户信息已并到侧边栏底部
