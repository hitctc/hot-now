...
- 长文写作时导语若为空或短于 20 字，Hermes 重试一次后才从正文提取有效片段；正文也不足时明确失败，不保存单字导语。短内容在终稿质检和审改后自动生成独立导语，模型失败时从终稿正文提取有效片段，本地成品保存后随成品推送到 HotNow。长短内容详情里的“生成新导语”共用 Hermes 生成入口，由 HotNow 唯一写入导语并同步正文，已写入但正文同步冲突时会单独提示；历史成品不会批量改写。
