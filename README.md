...
- 创作工作台：`/creative/source-items`、`/creative/finished-articles`、`/creative/short-source-items`、`/creative/short-finished-articles`、`/daily-digest`、`/monitor`。长短内容的素材与成品表格在移动端不固定左侧 ID/序号列，桌面端仍固定；右侧操作列保持原有行为。长短内容共用的成品详情弹窗在移动端铺满视口，桌面端维持原尺寸。
