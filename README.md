...
- 短内容成品支持代码制图片：服务端使用 SVG + Sharp 和随应用部署的 `NotoSansSC-Regular.otf` 生成 `2.5:1`、`1:1`、`3:4` 三张 PNG，写入 `code_image_cards`、人工正文和封面候选；Hermes 成品推送成功后通过 token 接口触发，页面也可在单篇详情中制作、重做、下载和复制图片地址。
