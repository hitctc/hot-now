      ...
      <div class="flex flex-col gap-6">
        ...
        <section v-if="displayTitles.length > 0 || (!props.readonly && regenTitleLoading)">
          <div class="mb-2 flex items-center justify-between">
            ...
            <div v-if="!props.readonly" class="flex items-center gap-3">
              ...
              <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="copyText(displayTitles.join('\n'))">复制全部</a-button>
            </div>
          </div>
          <ul class="m-0 list-none space-y-1 pl-0">
            ...
            >
              ...
              >编辑</button>
              <a-button v-if="!props.readonly && editingTitleIdx !== idx" type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="copyText(t)">复制</a-button>
            </li>
          </ul>
        </section>
        ...
        <section v-if="article.thesis">
          <div class="mb-2 flex items-center justify-between">
            ...
            <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="copyText(article.thesis!)">复制</a-button>
          </div>
        ...
        <section>
          <div class="mb-2 flex items-center justify-between">
            ...
            <div v-if="!props.readonly" class="flex items-center gap-3">
              ...
              <a-button v-if="displayIntros.length > 0" type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="copyText(displayIntros[activeIntroIndex] ?? '')">复制</a-button>
            </div>
          </div>
        ...
        <section v-if="displaySummaries.length > 0">
          <div class="mb-2 flex items-center justify-between">
            ...
            <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="copyText(displaySummaries[0] ?? '')">复制</a-button>
          </div>
        ...
        <section v-if="parseJsonArray(article.hooks).length > 0">
          <div class="mb-2 flex items-center justify-between">
            ...
            <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="copyText(parseJsonArray(article.hooks).join('\n'))">复制全部</a-button>
          </div>
          <ul class="m-0 list-none space-y-1 pl-0">
            <li
            ...
            >
              ...
              <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px] opacity-0 group-hover:opacity-100" @click="copyText(h)">复制</a-button>
            </li>
          </ul>
        </section>
        ...
        <section v-if="parseJsonArray(article.quotes).length > 0">
          <div class="mb-2 flex items-center justify-between">
            ...
            <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="copyText(parseJsonArray(article.quotes).join('\n'))">复制全部</a-button>
          </div>
        ...
        <section v-if="!props.readonly || (article?.comments && article.comments.length)">
          <div class="mb-2 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
              <a-button v-if="article?.comments && article.comments.length" type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="copyText((article?.comments ?? []).map(formatCommentPair).join('\n\n'))">复制全部</a-button>
              <a-button v-if="!props.readonly" type="link" size="small" class="!px-2 !py-1 !text-[11px]" :loading="generatingComments" :disabled="generatingComments" @click="handleGenerateComments">{{ generatingComments ? '生成中...' : (article?.comments && article.comments.length ? '重新生成评论' : '生成评论') }}</a-button>
            </div>
          </div>
        ...
        <section v-if="!props.readonly || (article?.authorExtensions && article.authorExtensions.length)">
          <div class="mb-2 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
              <a-button v-if="article?.authorExtensions && article.authorExtensions.length" type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="copyText((article?.authorExtensions ?? []).join('\n\n'))">复制全部</a-button>
              <a-button v-if="!props.readonly" type="link" size="small" class="!px-2 !py-1 !text-[11px]" :loading="generatingAuthorExtensions" :disabled="generatingAuthorExtensions" @click="handleGenerateAuthorExtensions">{{ generatingAuthorExtensions ? '生成中...' : (article?.authorExtensions && article.authorExtensions.length ? '重新生成拓展' : '生成拓展') }}</a-button>
            </div>
          </div>
          ...
          <div class="mb-2 flex items-center justify-between" data-editor-title>
            ...
            <template v-if="!props.readonly">
              <div class="flex flex-wrap items-center gap-2 max-[768px]:flex-nowrap max-[768px]:overflow-x-auto">
                ...
                <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="copyText(editContent)">复制原文</a-button>
                <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="copyMarkdownAsPlainText(editContent)">复制纯文本</a-button>
                <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="toggleSyncScroll">{{ syncScrollEnabled ? '同步滚动：开' : '同步滚动：关' }}</a-button>
                <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="toggleEditorFullscreen">{{ editorFullscreen ? '退出全屏' : '全屏' }}</a-button>
              </div>
            </template>
          </div>
          <!-- 只读模式：渲染后的 HTML 预览 -->
      ...
      >
        <div class="fullscreen-toolbar flex flex-col gap-2 border-b px-3 py-2 md:flex-row md:flex-wrap md:items-center md:justify-between md:gap-x-4 md:gap-y-2 md:px-4" style="border-color: var(--editorial-border);">
          ...
          <div class="flex flex-wrap items-center gap-x-2 gap-y-1 max-[768px]:flex-nowrap max-[768px]:overflow-x-auto">
            ...
            <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="copyMarkdownAsPlainText(editContent)">复制纯文本</a-button>
            <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="toggleSyncScroll">{{ syncScrollEnabled ? '同步滚动：开' : '同步滚动：关' }}</a-button>
            <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="handleSave" :loading="saving">保存</a-button>
            <a-button type="link" size="small" class="!px-2 !py-1 !text-[11px]" @click="toggleEditorFullscreen">退出全屏</a-button>
          </div>
        </div>
