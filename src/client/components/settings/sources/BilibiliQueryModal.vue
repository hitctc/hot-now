...
import type { BilibiliQueryFormState, BilibiliQueryModalMode } from "./sourcesPageShared";

defineProps<{
  open: boolean;
  mode: BilibiliQueryModalMode;
  form: BilibiliQueryFormState;
  error: string | null;
...
<template>
  <SettingsFormModal
    :open="open"
    :title="mode === 'create' ? '新增 B 站 query' : '编辑 B 站 query'"
    :width="680"
    @cancel="emit('cancel')"
  >
