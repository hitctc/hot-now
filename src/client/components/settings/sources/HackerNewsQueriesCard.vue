...
const emit = defineEmits<{
  add: [];
  collect: [];
  toggle: [query: SettingsHackerNewsQuery];
  ...
  >
    ...
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex flex-wrap gap-2">
        <a-button data-action="add-hackernews-query" @click="emit('add')">
          新增 Hacker News query
        </a-button>
        <a-button
          type="primary"
          data-action="manual-hackernews-collect"
          :disabled="!operations.canTriggerManualHackerNewsCollect || operations.isRunning"
          :loading="isActionPending('manual:hackernews-collect')"
          @click="emit('collect')"
        >
          {{ operations.isRunning ? "任务执行中..." : "手动采集 Hacker News" }}
        </a-button>
      </div>
    </div>
