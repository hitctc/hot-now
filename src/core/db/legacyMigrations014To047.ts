...
/** 按历史顺序执行 014–047；不拆单条 SQL，确保旧库升级仍处于同一事务。 */
export function applyLegacyMigrations014To047(db: SqliteDatabase): void {
    ...
    // 027: 封面图数组支持 — 增加 cover_image_index 字段记录用户选中的发布封面索引
    const coverImageIndexMigrationName = "add_cover_image_index";
    if (!hasColumn(db, "creative_finished_articles", "cover_image_index")) {
      db.exec(`ALTER TABLE creative_finished_articles ADD COLUMN cover_image_index INTEGER NOT NULL DEFAULT 0`);
    }
    ...
    db.prepare(
      `
        ...
        ON CONFLICT(version) DO NOTHING
      `
    ).run(27, coverImageIndexMigrationName);

    // 028: 标题选择支持 — 增加 title_index 字段记录用户选中的发布标题索引
    const titleIndexMigrationName = "add_title_index";
    if (!hasColumn(db, "creative_finished_articles", "title_index")) {
      db.exec(`ALTER TABLE creative_finished_articles ADD COLUMN title_index INTEGER NOT NULL DEFAULT 0`);
    }
    ...
    db.prepare(
      `
        ...
        ON CONFLICT(version) DO NOTHING
      `
    ).run(28, titleIndexMigrationName);
    ...
    // 038: 成品作者拓展评论（公众号+短内容共用，写作时生成）
    const authorExtensionsMigrationName = "038_finished_articles_author_extensions";
    if (!hasColumn(db, "creative_finished_articles", "author_extensions")) {
